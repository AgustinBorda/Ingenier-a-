  ...
  static Object preg_id;
    ...
    public static void main( String[] args )
    {
      ...
      post("/categoryquestion", (req,res) -> {
        ...
        questions = Question.findBySQL("SELECT * FROM questions WHERE id NOT IN (SELECT id FROM questions NATURAL JOIN user_questions WHERE user_id = ?) AND category = ?",currentUser.get("id"),bodyParams.get("category"));
      ...
      get("/question", (req,res) -> {
        ...
        Map<String, Object> bodyParams = new Gson().fromJson(req.body(), Map.class);
        List<Question> questions;
        questions = Question.findBySQL("SELECT * FROM questions WHERE id NOT IN (SELECT id FROM questions NATURAL JOIN user_questions WHERE user_id = ?)",currentUser.get("id"));
        Question question = questions.get(r.nextInt(questions.size()));
        List<Option> options = Option.where("question_id = ?", question.get("id"));
        preg_id = question.get("id");
        resp.put("description",question.get("description"));;
